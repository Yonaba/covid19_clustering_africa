...
lats <- abs(lats)
conf_pm <- read.csv("data_completion/3_data_final.csv", header = T, sep = ",", dec = ".")$conf_pm
death_pm <- read.csv("data_completion/3_data_final.csv", header = T, sep = ",", dec = ".")$death_pm
...
p1 <- ggplot(df, aes(x=lats, y=conf_pm)) +
  annotate("text", x = 28, y = 5, label = paste0("y = ",round(lm(conf_pm ~lats, df)$coefficients[2], 3),
                          "x +", round(lm(conf_pm ~ lats, df)$coefficients[1], 3)), fontface = 2) +
  annotate("text", x = 30, y = 4.25, label = paste0("r²: ",round(r2_conf, 3)), fontface = 4) +  
  annotate("text", x = 0, y = 12.5, label = "a)", fontface = 2) +
  geom_point() +
  labs(x = "Absolute latitude (°)", y = "log(Cumulative cases per million)") + 
...
p2 <- ggplot(df, aes(x=lats, y=death_pm)) +
  ...
  annotate("text", x = 30, y = 0.35, label = paste0("r²: ",round(r2_death, 3)), fontface = 4) +   
  annotate("text", x = 0, y = 7.8, label = "b)", fontface = 2) +
  geom_point() +
  labs(x = "Absolute latitude (°)", y = "log(Cumulative deaths per million)") + 
  geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) +
  theme_ipsum_pub()
